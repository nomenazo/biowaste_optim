function dydt=compostingtestW(t,y);
...
%Water module
Mw_H2O = 18e-3;      % kg/mol
klh2O  = 1e-4;       % evaporation coefficient (Petric 2008)
Aexposed = Vtotal^(2/3); % surface d’échange approx (m2)

%% --- Gas phase water ---
Pwvap = (Wvap/Mw_H2O) * R * T / Vgas;   % Pa
Ptot  = (ntot*R*T)/Vgas;               % Pa

% Saturation vapour pressure (Petric 2008, T in K)
Ps = 10^(22.443 - 2795/T - 1.6798*log10(T)); % Pa
Ps = max(Ps,0);


%dWvapdt = (klh2O/TM) * (Ps - Pwvap) - Qair*Wvap/(rhoair *Vgas*TM);
%% --- Evaporation & ventilation ---
RH = min(max(Pwvap/Ps,0),0.99); % relative humidity

% Mass transfer coefficient (kg/s/Pa)
k_evap = klh2O * Aexposed / (R*T);

% Evaporation flux
fW = W / (W + 0.3);

% Driving force (bounded)
%% --- Evaporation model (h-1 consistent) ---

% Driving force (Pa)
DeltaP = max(Ps - Pwvap, 0);

% Evaporation kinetic coefficient (h-1·Pa-1)
k_evap_h = 1e-6;   % << paramètre clé à calibrer

% Maximum evaporation rate (kgH2O / kgTM / h)
Emax = 0.003;      % 0.3 % eau / h (≈ 7 % / jour MAX)

% Evaporation rate (kgH2O / kgTM / h)
Evap = Emax * (k_evap_h * DeltaP) / (Emax + k_evap_h * DeltaP);        % kg/s (consistent with gas balance)

dWvapdt = Evap - 0.3 * Qair * (Pwvap / Ptot);

Wbio = (((Ymb_c_h)*v13+(Ymb_p_h)*v14+(Ymb_l_h)*v15+(Ytb_c_h)*v16+(Ytb_p_h)*v17+(Ytb_l_h)*v18+(Yma_c_h)*v19+(Yma_p_h)*v20+(Yma_l_h)*v21+...
...
dWdt = Wbio - Evap  + 2.25*Vmax*((CH4gen/W)/(km+(CH4gen/W)))*((O2diss/(W*32e-3))/(Kch4_O2+(O2diss/(W*32e-3)))); %Petric 2005 + 0.842*v45
